...
func (l *ArticleLogic) articleOperate(ctx context.Context, msg *types.CanalArticleMsg) error {
	...
	for _, d := range msg.Data {
		...
		t, err := time.Parse("2006-01-02 15:04:05", d.PublishTime)
		if err != nil {
			logx.Errorf("time.Parse time: %s error: %v", d.PublishTime, err)
			continue
		}
		...
		case types.ArticleStatusVisible:
			...
			if b {
				_, err = l.svcCtx.BizRedis.ZaddCtx(ctx, publishTimeKey, t.Local().Unix(), d.ID)
				if err != nil {
